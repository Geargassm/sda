...
public class MediumwormRenderer extends MobRenderer<MediumwormEntity, Modelsmallworm<MediumwormEntity>> {
	public MediumwormRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(Modelsmallworm.LAYER_LOCATION)), 0.6f);
	}

	@Override
	protected void scale(MediumwormEntity entity, PoseStack poseStack, float f) {
		poseStack.scale(2.5f, 2.5f, 2.5f);
	}

	@Override
	public ResourceLocation getTextureLocation(MediumwormEntity entity) {
		return new ResourceLocation("factorio_enemies:textures/entities/mediumspitters.png");
	}

	private static final class AnimatedModel extends Modelsmallworm<MediumwormEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<MediumwormEntity>() {
			@Override
			...
			@Override
			public void setupAnim(MediumwormEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
