...
public class NestspittersMobEntity extends Monster {

    private static final EntityDataAccessor<Integer> TIER =
            SynchedEntityData.defineId(NestspittersMobEntity.class, EntityDataSerializers.INT);

    private static final int TERRITORY_RADIUS = 64;
    private static final int MAX_SPITTERS = 10;
    ...
    public NestspittersMobEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(FactorioEnemiesModEntities.NESTSPITTERS_MOB.get(), world);
    }

    public NestspittersMobEntity(EntityType<NestspittersMobEntity> type, Level world) {
        super(type, world);
        setMaxUpStep(0f);
        xpReward = 0;
        setNoAi(false);
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<Player>(this, Player.class, false, true) {
            ...
            public boolean canUse() {
                Player nearest = NestspittersMobEntity.this.level().getNearestPlayer(
                        NestspittersMobEntity.this.getX(), NestspittersMobEntity.this.getY(), NestspittersMobEntity.this.getZ(),
    ...
    private void spawnSpitters() {
        ...
        int spitterCount = serverLevel.getEntitiesOfClass(SmallspittersEntity.class, searchBox).size()
                + serverLevel.getEntitiesOfClass(MediumspittersEntity.class, searchBox).size()
                + serverLevel.getEntitiesOfClass(BigspittersEntity.class, searchBox).size()
                + serverLevel.getEntitiesOfClass(BehemothspittersEntity.class, searchBox).size();

        if (spitterCount >= MAX_SPITTERS) return;

        RandomSource random = serverLevel.getRandom();
        int tier = getTier();
        EntityType<?> entityType = getSpitterTypeForTier(tier, random);
    ...
    private EntityType<?> getSpitterTypeForTier(int tier, RandomSource random) {
        return switch (tier) {
            case 1 -> random.nextBoolean()
                    ? FactorioEnemiesModEntities.SMALLSPITTERS.get()
                    : FactorioEnemiesModEntities.MEDIUMSPITTERS.get();
            case 2 -> FactorioEnemiesModEntities.MEDIUMSPITTERS.get();
            case 3 -> random.nextBoolean()
                    ? FactorioEnemiesModEntities.MEDIUMSPITTERS.get()
                    : FactorioEnemiesModEntities.BIGSPITTERS.get();
            case 4 -> FactorioEnemiesModEntities.BIGSPITTERS.get();
            case 5 -> random.nextBoolean()
                    ? FactorioEnemiesModEntities.BIGSPITTERS.get()
                    : FactorioEnemiesModEntities.BEHEMOTHSPITTERS.get();
