...
public class BigwormRenderer extends MobRenderer<BigwormEntity, Modelsmallworm<BigwormEntity>> {
	public BigwormRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(Modelsmallworm.LAYER_LOCATION)), 0.6f);
	}

	@Override
	protected void scale(BigwormEntity entity, PoseStack poseStack, float f) {
		poseStack.scale(4f, 4f, 4f);
	}

	@Override
	public ResourceLocation getTextureLocation(BigwormEntity entity) {
		return new ResourceLocation("factorio_enemies:textures/entities/bigspitters.png");
	}

	private static final class AnimatedModel extends Modelsmallworm<BigwormEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<BigwormEntity>() {
			@Override
			...
			@Override
			public void setupAnim(BigwormEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
